...
var _ = Describe("CurrencyConverter", func() {
	Describe("Exchange", func() {
		...
		BeforeEach(func() {
			currencyConverter = NewCurrencyConverter(api)
		})
		...
		Context("when the internet is down", func() {
			BeforeEach(func() {
				api := "httpi.fixer.io/latest?base=GBP&HJHH"
				currencyConverter = NewCurrencyConverter(api)
			})

			It("uses the ballback rate for USD", func() {
			...
			It("uses the ballback rate", func() {
				Expect(currencyConverter.Exchange(3).USD).To(BeNumerically("==", 4.68))
			})
		})
	})

	Describe("NewRates", func() {
		var currencyConverter *CurrencyConverter
		BeforeEach(func() {
			currencyConverter = NewCurrencyConverter(api)
		})
		...
		It("should use a new set of rates", func() {
			currencyConverter.NewRates()
		})
	})
})
